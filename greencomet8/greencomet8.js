...
var messagesRef = firebase.database().ref("botform");
 ...
 let loadIntervalfunction loader(element) {element.textContent = " "
 ...
 let interval = setInterval(() => {if (index < text.length) {element.innerHTML += text.charAt(index)index++ } else { clearInterval(interval) }}, 20)}
 ...
 e.preventDefault()
 const data = new FormData(form)chatContainer.innerHTML += chatStripe(false,data.get("prompt"))
 form.reset()
 const uniqueId = generateUniqueId()chatContainer.innerHTML += chatStripe(true, " ", uniqueId)
...
const parsedData = data.bot.trim()
 ...
 messageDiv.innerHTML = "Something went wrong"alert(err)}}
